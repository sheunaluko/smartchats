...
test.describe('Agent Wait', () => {
  test('fork_process agent waits 12s then responds', async () => {
    test.setTimeout(90_000);

    // Open-mode (LocalAuthProvider): no sign-in required. Ephemeral browser.
    ...
    try {
      ...
      await page.waitForFunction(
        () => (window as any).__smartchats__?.simi?.workflows?.agent_wait_flow,
        { timeout: 30_000 },
      );

      const result = await page.evaluate(() => {
        return (window as any).__smartchats__.simi.workflows.agent_wait_flow();
      });
      ...
      await captureSession(page, 'agent_wait');
    } catch (err) {
      await captureSession(page, 'agent_wait_FAIL');

      await page.waitForTimeout(3000);
      await page.screenshot({ path: 'test-results/agent-wait-debug.png', fullPage: true });
