...
import {
  ...
  type MobileVoiceViewModel,
  type VoiceMoment,
} from '../types/mobileVoice';
...
export function MobileVoiceShell({ voice, ui, settings, widgetConfig, widgetProps, actions, meta }: ShellProps) {
  ...
  const vm: MobileVoiceViewModel = useMemo(() => {
    ...
    if (currentExecution?.error) {
    ...
    } else if (widgetProps.functionCalls.length > 0) {
      ...
      moments.push({
        ...
        body: 'The assistant invoked a tool as part of the response.',
        meta: latestCall.duration ? `${latestCall.duration}ms` : 'Tool call',
      });
    }
  ...
  return (
    <div className="flex min-h-dvh flex-col overflow-hidden bg-[var(--sc-background)]">
      ...
      <main className="flex flex-1 flex-col px-4 pb-4">
        <div className="mx-auto flex w-full max-w-[28rem] flex-1 flex-col justify-center gap-6">
          <VoiceStage
            state={vm.state}
            level={vm.level}
            variant="orb"
          ...
          <div className="mx-auto flex min-h-[10.5rem] w-full max-w-[28rem] flex-col gap-3">
      ...
      />
      <ActionRail
        onKeyboard={() => setComposerOpen((value) => !value)}
      />
      <FallbackComposer
        open={composerOpen}
