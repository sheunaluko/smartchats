...
export function MobileVoiceStreamShell({ voice, ui, settings, widgetConfig, widgetProps, actions, meta }: ShellProps) {
  ...
  const vm: MobileVoiceViewModel = useMemo(() => {
    ...
    if (state === 'processing') {
      moments.push({
        ...
        body: 'Evaluating the request and shaping the next action.',
        meta: 'Live analysis',
      });
    }

    if (widgetProps.functionCalls.length > 0) {
      const latestCall = widgetProps.functionCalls[widgetProps.functionCalls.length - 1];
      moments.push({
        id: latestCall.callId || 'latest-call',
        kind: 'action',
        title: `Using ${latestCall.name}`,
        body: 'A tool call is part of the current response.',
        meta: latestCall.duration ? `${latestCall.duration}ms` : 'Tool activity',
      });
    }

    if (currentExecution?.error) {
      moments.push({
        id: `${currentExecution.executionId}-error`,
        kind: 'result',
        title: 'Execution error',
        body: currentExecution.error,
        meta: 'Latest execution failed',
      });
    } else if (currentExecution?.result !== undefined) {
      moments.push({
        id: `${currentExecution.executionId}-result`,
        kind: 'result',
        title: 'Execution result',
        body: summarizeResult(currentExecution.result),
        meta: currentExecution.duration ? `${currentExecution.duration}ms` : 'Completed',
      });
    }
      ...
      <main className="flex flex-1 flex-col px-4 pb-4">
        <div className="mx-auto flex w-full max-w-[28rem] flex-1 flex-col justify-center gap-5">
          ...
          <div className="space-y-2">
            {vm.moments.map((moment, index) => (
              <DataCard
                key={moment.id}
                tone={moment.kind === 'result' ? 'primary' : moment.kind === 'confirmation' ? 'warning' : 'default'}
                className={index === 0 ? 'animate-sc-slide-in-up' : ''}
                header={<div className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-sc-text-muted">{moment.title}</div>}
              >
                <p className="text-sm leading-relaxed text-sc-text">{moment.body}</p>
                {moment.meta && <p className="text-xs text-sc-text-muted">{moment.meta}</p>}
              </DataCard>
            ))}
          </div>
