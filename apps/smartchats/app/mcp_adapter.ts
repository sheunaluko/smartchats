...
const log = logger.get_logger({ id: "mcp_adapter" });
...
async function connect_to_mcp_server(url: string): Promise<Client> {
  log(`Connecting to MCP server: ${url}`);
  ...
  try {
    ...
    log(`Successfully connected to MCP server`);
    return client;
  } catch (error: any) {
    log(`Failed to connect to MCP server: ${error.message}`);
    throw error;
  }
}
...
async function list_mcp_tools(client: Client): Promise<MCPTool[]> {
  log(`Listing tools from MCP server`);

  try {
    const response = await client.listTools();
    const tools = response.tools || [];
    log(`Discovered ${tools.length} tools from MCP server`);
    log(`Raw tools response: ${JSON.stringify(tools, null, 2)}`);
    return tools as MCPTool[];
  } catch (error: any) {
    log(`Failed to list MCP tools: ${error.message}`);
    throw error;
  }
}
...
async function call_mcp_tool(
  ...
  log(`Calling MCP tool: ${toolName} with args: ${JSON.stringify(args)}`);
  ...
  try {
    ...
    log(`MCP tool call succeeded: ${toolName}`);
    ...
    log(`MCP tool call failed: ${toolName} - ${error.message}`);
    throw error;
  }
}
...
function convert_mcp_schema_to_cortex_parameters(
  ...
  log(`Converting schema: ${JSON.stringify(inputSchema)}`);

  if (!inputSchema.properties) {
    log(`No properties found in schema, returning null`);
    return null;
  }
  ...
  log(`Converted parameters: ${JSON.stringify(cortexParams)}`);
...
function coerce_argument_types(
  ...
  log(`Coerced arguments: ${JSON.stringify(coercedArgs)}`);
  return coercedArgs;
}
...
function convert_mcp_tool_to_cortex_function(
  ...
  log(`Converting MCP tool to Cortex function: ${mcpTool.name}`);
  log(`Tool description: ${mcpTool.description}`);
  log(`Tool inputSchema: ${JSON.stringify(mcpTool.inputSchema, null, 2)}`);
  ...
  const cortexFunction = {
    ...
    fn: async (ops: any) => {
      ...
      log(`Raw arguments before coercion: ${JSON.stringify(actualArgs)}`);
      ...
      try {
        ...
        log(`MCP tool execution error: ${mcpTool.name} - ${error.message}`);
  ...
  log(`Created Cortex function: ${JSON.stringify({
...
export async function create_cortex_functions_from_mcp_server(
  ...
  log(`Starting MCP adapter for server: ${url}`);
  ...
  log(`MCP adapter complete: Created ${cortexFunctions.length} Cortex functions`);
...
export async function create_cortex_function_for_mcp_tool(
  ...
  log(`Creating Cortex function for specific MCP tool: ${toolName}`);
  ...
  if (!tool) {
    log(`MCP tool not found: ${toolName}`);
    return null;
  }
...
export async function create_cortex_functions_from_mcp_server_filtered(
  ...
  log(`Creating filtered Cortex functions with pattern: ${namePattern}`);

  const client = await connect_to_mcp_server(url);
  const mcpTools = await list_mcp_tools(client);

  const filteredTools = mcpTools.filter((tool) => namePattern.test(tool.name));
  log(`Filtered to ${filteredTools.length} tools matching pattern`);
