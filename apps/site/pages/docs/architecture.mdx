...
- `local_date: string` — `YYYY-MM-DD` as the user perceived it in their tz, precomputed so SurrealDB `GROUP BY local_date` does daily aggregation correctly without any timezone logic at query time.
...
Schema-change history lives in `HISTORY.md` at the repo root.
