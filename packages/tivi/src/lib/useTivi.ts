...
export function useTivi(options: UseTiviOptions): UseTiviReturn {
  ...
  const ttsCallFn = useMemo(() => {
    // 1. External ttsCallFn takes priority (SmartChats backward compat)
    if (externalTtsCallFn) return externalTtsCallFn;
    // 2. Browser TTS — no call function needed
    if (settings.ttsProvider !== 'openai') return undefined;
    // 3. Local backend — uses OAAK from localStorage
    if (settings.ttsBackend === 'local') return createLocalTtsCallFn();
    // 4. Firebase backend with callable provided
    if (settings.ttsBackend === 'firebase' && firebaseTtsCallable) {
      return createFirebaseTtsCallFn(firebaseTtsCallable);
    }
    // 5. Firebase backend but no callable — fallback to local with warning
    if (settings.ttsBackend === 'firebase') {
      log('ttsBackend is "firebase" but no firebaseTtsCallable provided — falling back to local');
      return createLocalTtsCallFn();
    }
    return undefined;
  }, [externalTtsCallFn, settings.ttsProvider, settings.ttsBackend, firebaseTtsCallable]);
  ...
  useEffect(() => {
    onQueueFirstUtteranceRef.current = onQueueFirstUtterance;
  }, [onQueueFirstUtterance]);

  useEffect(() => {
    onQueueEntryCompleteRef.current = onQueueEntryComplete;
  }, [onQueueEntryComplete]);
  ...
  useEffect(() => {
    const queue = createTTSSpeechQueue({
      ttsCallFn,
      ttsStreamCallFn,
      speakFn: !ttsCallFn ? (text, rate) => tts.speakWithRate(text, rate) : undefined,
      cancelFn: !ttsCallFn ? () => tts.cancelSpeech() : undefined,
      onStart: () => {
        setIsSpeaking(true);
        isSpeakingRef.current = true;
        // Pause recognition during TTS
        recognitionRef.current?.pause();
        // Resume VAD for interrupt detection (responsive/continuous modes)
        if (enableInterruptionRef.current && modeRef.current !== 'guarded' && vadRef.current) {
          log('Queue onStart: resuming VAD for interrupt detection');
      ...
      onDrain: ({ cancelled }) => {
        ...
        if (isListeningRef.current && recognitionRef.current) {
          if (modeRef.current === 'continuous') {
            log('Queue onDrain: restarting recognition (continuous mode)');
            recognitionRef.current.start();
          }
        }
        onQueueDrainRef.current?.({ cancelled });
      },
      onError: (err) => {
        log(`TTS queue error: ${err.message}`);
      },
      onStateChange: (entries) => {
        onQueueStateChangeRef.current?.(entries);
      },
      onFirstUtterance: () => {
        onQueueFirstUtteranceRef.current?.();
      },
  ...
  const speak = useCallback(async (text: string, rate: number = 1.0) => {
    ...
    if (ttsQueueRef.current) {
      ...
      ttsQueueRef.current.speakText(text);
      return;
    }

    // Fallback: no queue (shouldn't happen, but safe)
  ...
  const speakCached = useCallback((audioBuffer: AudioBuffer) => {
    if (ttsQueueRef.current) {
      ttsQueueRef.current.speakCached(audioBuffer);
    }
  }, []);

  const cancelSpeech = useCallback( ()=> {
  	tts.cancelSpeech();
  	// Also cancel the queue
  	ttsQueueRef.current?.cancel();
  },[]);
