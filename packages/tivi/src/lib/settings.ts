...
const DEPRECATED_TTS_MODELS = ['tts-1', 'tts-1-hd'];
  ...
  load(): TiviSettings {
    ...
    try {
      ...
      if (raw) {
        ...
        // Migrate deprecated TTS model names
        if (parsed.openaiModel && DEPRECATED_TTS_MODELS.includes(parsed.openaiModel)) {
          parsed.openaiModel = TIVI_DEFAULTS.openaiModel;
        }
