...
const dlog = logger.get_logger({ id: 'ts_debug' });

// Expose logger to browser console for easy log export
if (typeof window !== 'undefined') {
  (window as any).tslog = logger;
}
...
export function useCalibration(
  ...
  const startPhase2 = useCallback(async () => {
    ...
    // Resume VAD so we can measure echo leakage during TTS playback
    tivi.resumeVADProcessing();

    // Play TTS — fire-and-forget to queue, then await drain
    dlog('startPhase2: threshold=', phase1Results.positiveSpeechThreshold, 'playing TTS...');
    tivi.speak(CALIBRATION_TTS_TEXT, 1.0);
    await tivi.ttsQueue?.waitUntilDrained();
