...
function resolveOne(resolver: Resolver, getState: () => any, results: Record<string, any>): any {
  ...
  switch (resolver.$resolve) {
    ...
    case 'eval': {
      return resolver.fn(state, results);
    }

    case 'result': {
      const value = results[resolver.step];
      if (value === undefined) {
        throw new Error(`result resolver: no result for step "${resolver.step}"`);
      }
      if (resolver.path) {
        const nested = getPath(value, resolver.path);
        if (nested === undefined) {
          throw new Error(`result resolver: path "${resolver.path}" not found in result for step "${resolver.step}"`);
        }
        return nested;
      }
      return value;
 ...
 */
export function resolveArgs(args: any[], getState: () => any, results: Record<string, any> = {}): { resolved: any[]; resolverEvents: Array<{ resolver: Resolver; status: 'ok' | 'error'; error?: string }> } {
  ...
  const resolved = args.map(arg => {
    ...
    try {
      const value = resolveOne(arg, getState, results);
