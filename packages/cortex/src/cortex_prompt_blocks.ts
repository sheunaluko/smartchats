...
8. NEVER use blocking loops or busy-waits (e.g. while(Date.now() - start < N) {}).
   These freeze the thread and break the runtime. For delays, ALWAYS use:
   await new Promise(resolve => setTimeout(resolve, milliseconds));

9. IMPORTANT - Turn-Based Execution:
...
10. NO OPTIMISTIC EXECUTION: Never assume an async operation succeeded.
   Always return the result to yourself and verify it in the next turn before
   reporting success to the user. If a function returns an error, handle it —
   do not tell the user it worked.

11. BATCH DATA GATHERING: Maximize what you collect in a single code turn.
   If multiple queries are independent (e.g. initialization: fetch instructions,
   metrics context, recent logs), run them all with Promise.all and return the
   combined result for review before responding.

   GOOD (one turn, all data):
     [instructions, metrics, logs] = await Promise.all([
       get_procedural_instructions({}),
       get_metrics_context({}),
       get_recent_logs({limit: 5})
     ]);
     return {instructions, metrics, logs};

   BAD (three separate turns for independent queries):
     // Turn 1: return await get_procedural_instructions({});
     // Turn 2: return await get_metrics_context({});
     // Turn 3: return await get_recent_logs({limit: 5});

   Only use sequential turns when a later call depends on the result of an earlier one.
...
export const cortexOutputFormat = {
    types: `
You receive user data messages and return CortexOutput messages, defined in TypeScript as:
...
export const codeOutputFormat = {
    types: `
You receive user data messages and return CodeOutput messages, defined in TypeScript as:
