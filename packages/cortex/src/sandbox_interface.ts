...
export interface SandboxExecutor {
  ...
  /**
   * Sync workspace into the running sandbox mid-execution (optional).
   * Pushes updated workspace into the sandbox so code reads see live values
   * instead of a stale snapshot from execution start.
   */
...
export interface SandboxResult<T = any> {
  ...
  events: SandboxEvent[]
  duration?: number
  /** Diagnostic data included on execution errors for post-session debugging */
  diagnostics?: {
    srcdoc: string
    code: string
    context_keys: string[]
