...
/**
 * Diagnostic log that only fires when `window.__SIMI_DEBUG__` is truthy.
 * Intended for trace points that are useful during Simi workflow runs but
 * too noisy for normal development. Safe to call from anywhere — no-ops
 * outside the browser and when the flag is unset, so production bundles
 * carry negligible overhead.
 *
 * Opt in:
 *   - Test harness: Playwright sets `window.__SIMI_DEBUG__ = true` via
 *     `addInitScript` before page navigation (gated on `SIMI_DEBUG=1` env).
 *   - Browser console: `window.__SIMI_DEBUG__ = true` (no reload needed —
 *     flag is checked per-call).
 */
export function simi_debug(...args : any[]) : void {
  if (typeof window === 'undefined') return ;
  if (!(window as any).__SIMI_DEBUG__) return ;
  console.log('[simi_debug]' , ...args) ;
}
