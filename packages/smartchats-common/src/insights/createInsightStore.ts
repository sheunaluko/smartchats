...
export function createInsightStore<T extends object>(
  ...
  if (typeof window !== 'undefined') {
    ...
    if (config.workflows) {
      ...
      // Build the registry keyed by workflow.id (not by config key) so
      // `setupWorkflows: ['foo']` resolves by the workflow's declared id.
      const registry: Record<string, SimiWorkflow> = {};
      for (const workflow of Object.values(config.workflows)) {
        registry[workflow.id] = workflow;
      }

      for (const workflow of Object.values(config.workflows)) {
        compiledWorkflows[workflow.id] = async (opts?) => {
          const mergedOpts: RunOpts = {
            ...opts,
            workflowRegistry: { ...registry, ...(opts?.workflowRegistry ?? {}) },
          };
          const result = await executeWorkflow(workflow, dispatch, useStore.getState, () => _client, mergedOpts);
          // Flush remaining events before Playwright tears down the page
          if (_client && typeof (_client as any).flushBatch === 'function') {
            await (_client as any).flushBatch();
          }
          return result;
        };
      }
