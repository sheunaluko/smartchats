...
const asnc = { wait: (ms: number) => new Promise<void>((r) => setTimeout(r, ms)) };
const fp = {
    nth: <T>(xs: T[] | string, n: number) => (xs as any)[n],
    format: (tpl: string, args: any[]) => {
        let i = 0;
        return tpl.replace(/\{\}/g, () => String(args[i++]));
    },
};
...
/** Call from a user gesture (click/tap) to unlock AudioContext on iOS Safari. */
