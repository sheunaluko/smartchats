...
/**
 * audit_slow-calls — DB-side slow-function-call surfacer.
 *
 * Lists every function call whose duration crossed --threshold-ms, sorted
 * longest first. Surfaces real-world patterns like accumulate_text calls
 * sitting for minutes (user paused mid-recording) and 1-hour iframe
 * sandbox cap trips.
 *
 * Usage:
 *   npm run audit:slow-calls -- [options]
 *
 * Options:
 *   --threshold-ms <n>    Minimum duration (ms). Default 30000.
 *   --name <names>        Comma-separated list of function names to include.
...
const USAGE = `Usage: audit_slow-calls [options]
  --threshold-ms <n>    Minimum duration (ms). Default 30000.
  --name <names>        Comma-separated function names to include.
...
const result = await querySlowFunctionCalls(client, {
    since: args.since,
    until: args.until,
    app: args.app,
    userId: args.user,
    ...
    limit: args.limit,
    minDurationMs: args.thresholdMs,
    nameFilter: args.name,
});

const text = formatSlowCalls(result, { format: args.format });
